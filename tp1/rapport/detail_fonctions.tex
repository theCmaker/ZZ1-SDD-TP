\section{Gestion des news}
  ...
  \subsection{traiter\_elt\_date}
    \begin{algo}[informal]
      \ALGO{traiter\_elt\_date (Principe)}
      \BEGIN
        ...
        \WHILE {la date de début de l'élément courant est égale à la date recherchée}
          \STATE{On applique la fonction passée en paramètre à la cellule courante}
          \STATE{On passe à la cellule suivante}
        \ENDWHILE
      \END
    \end{algo}
  ...
  \subsection{afficher\_messages\_date}
    \begin{algo}[informal]
      ...
      \BEGIN
        \STATE{On appelle la fonction \textit{traiter\_elt\_date} avec la date de début des messages à afficher et l'adresse de la fonction \textit{afficher\_message}}
      \END
    \end{algo}
