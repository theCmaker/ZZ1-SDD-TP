Dans cette partie, sont décrits les algorithme de principes associés aux fonctions écrites en langage C, ainsi qu'un lexique concernant les variables intermédiaires des fonctions.
Le lexique des variable d'entré, sortie et entré/sortie sont disponibles dans le code source directement
...
%        \ENDWHILE
        \WHILE{la liste n'est pas finie}
          \WHILE{la date de fin de l'élément courant est ultérieure à la date recherchée}
            ...
            \STATE{On passe à la cellule suivante}
          \ENDWHILE
%    \end{algo} %pourquoi ces 2 lignes ?
%    \begin{algo}[continue]
  ...
  \subsection{remplacer\_date}
    \begin{algo}[informal]
      ...
      \BEGIN
        ...
        \WHILE{l'élémént courant n'est pas NULL ET ALORS La date de début de message de l'élément courant est supérieure à la date à modifier}
          \STATE{On passe à l'élément suivant}
        \ENDWHILE
        \WHILE{l'élément courant n'est pas NULL ET ALORS La date de début de message de l'élément courant est égale à la date à modifier}
...
\section{Gestion de la liste chaînée}
  ...
  \subsection{rech\_prec}
    \begin{algo}[informal] %principe
      ...
      \BEGIN
        ...
        \WHILE{*prec n'est pas NULL ET ALORS la date de début de *prec est supérieure à la date après laquelle insérer}
          \STATE{prec prend l'adresse de l'adresse de l'élément suivant dans la liste}
        \ENDWHILE
        \STATE{La variable d'entré/sortie \textit{existe} prend la valeur 1 si la date cherchée est présente, 0 sinon}
        \RETURN{prec}
      \END
    \end{algo}
  ...
  \subsection{ins\_cell}
    \begin{algo}[informal] %principe
      ...
      \BEGIN
        \STATE{On récupère un pointeur sur l'adresse de l'élément avant lequel insérer avec la fontion \textit{rech\_prec}}
    ...
    \begin{algo}[informal] %lexique
      \VAR
        \DECLVAR{existe}{booleen de présence de l'élément recherché, modifié dans \textit{rech\_prec}}
  ...
  \subsection{creer\_cell}
    \begin{algo}[informal] %principe
      ...
      \BEGIN
        ...
        \IF{l'allocation a réussie}
          \STATE{On place les dates de début et de fin de message dans les champs appropriés}
          \STATE{On allou l'espace nécéssaire pour le message dans le champ texte}
          \STATE{On recopie le texte passé en entré dans le champ texte de la cellule}
        \ENDIF
    ...
    \begin{algo}[informal] %lexique
      \VAR
        \DECLVAR{*elt}{pointeur sur le nouvel élément crée, ce sera également le retour de la fonction. Si celui-ci est à NULL, cela signifie qu'il y a eu un soucis lors de son allocation}
      \ENDVAR
    \end{algo}
