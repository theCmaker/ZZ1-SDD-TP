  ...
  \subsection{rech\_mot}
    \begin{algo}[informal] %principe
      ...
      \BEGIN
        ...
        \WHILE[Recherche du début du mot]{existe \EQ Vrai \AND arbre \NEQ \NIL{} \AND cour est en minuscule}
    ...
    \begin{algo}[informal] %principe
      ...
      \BEGIN
        ...
        \IF{Le mot n'est pas vide}
          ...
          \STATE[Avec un simple Tant Que]{On passe chaque lettre de cette copie en minuscule, sauf la dernière}
          \STATE{Lance rech\_mot() et stocke les résultats dans arbre et cour}
          \IF[Le mot n'est pas présent]{\NOT fin de chaîne}
            \IF{arbre non vide \AND valeur du n\oe ud \EQ cour(en minuscule)}
            ...
            \ELSE
              ...
              \IF{élément correctement créé}
                ...
                \STATE{Avance d'un caractère dans le mot}
                \WHILE[Insère les lettres restantes]{code d'erreur \NEQ 0 \AND cour \NEQ '\textbackslash 0'}
                  ...
                  \ELSE
                    \STATE[Problème d'allocation]{Code d'erreur passe à 0}
                  \ENDIF
                \ENDWHILE
              \ELSE
                \STATE[Problème d'allocation]{Code d'erreur passe à 0}
